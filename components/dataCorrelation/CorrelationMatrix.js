...
const CorrelationMatrix = ({ }) => {
    return (
        <Box sx={{ height: "83vh", width: "100%", p: 3,pb:1, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <TableContainer sx={{ height: "100%", overflow: "auto", border: "1px solid #c1c1c1", borderRadius: "3px", "&::-webkit-scrollbar": { width: "0.6rem", height: "0.6rem", borderRadius: "2rem" }, "&::-webkit-scrollbar-track": { bgcolor: "#e1e1e1" }, "&::-webkit-scrollbar-thumb": { bgcolor: "#a1a1a1", borderRadius: "3rem" } }}>
                <Table aria-label="sticky table">
                    ...
                    <TableBody>
                        {rows.map((row) => {
                            return (
                                <TableRow role="checkbox" tabIndex={-1} key={row.name} sx={{ height: "5rem" }}>
                                    {columns.map((column) => {
